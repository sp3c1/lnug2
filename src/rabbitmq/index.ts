...
async function bootstrap() {
  try {
    ...
    // actuall publishing
    await publishers.publish('test', 'replyToQueue', { expiration: 10000, priority: 0 }, 'this is my delayed Message 1');
    await publishers.publish('test', 'replyToQueue', { expiration: 8000, priority: 0 }, 'this is my delayed Message 2');
    await publishers.publish('test', 'replyToQueue', { expiration: 4000, priority: 0 }, 'this is my delayed Message 3');
    await publishers.publish('test', 'replyToQueue', { expiration: 1000, priority: 0 }, 'this is my delayed Message 4');

    await consumers.subscribe('test2', {}, 10, async (msg, msgRaw, ack) => {
